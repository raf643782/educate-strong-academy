...
**Last updated:** Priority 3 Final Verification (Master Continuation Programme — Knowledge Hub and EatStrong Editorial Completion) — 2026-07-23. **Priority 3 remains OPEN.** PR #3 (EatStrong disclaimer) is merged and fully verified live in production. The Knowledge Hub true-404 was verified using an owner-supplied Vercel Shareable Link — and it fails: the route returns HTTP 200, not 404. Root cause: the project's real Vercel deployments do not appear to honour the root-level `vercel.json`'s custom routing at all (a pre-existing condition, also affecting the Exercise/Event 404 mechanism and the `start-strongman-safely` redirect). See the final verification findings below.
...
### 2. Knowledge Hub true 404 — verification now UNBLOCKED, and it FAILS ❌

You supplied a Vercel Shareable Link (`?_vercel_share=...`) for the `feature/libraryPages` preview, which successfully bypassed the SSO wall from the prior report. With direct access, the true-404 requirement does **not** pass, and the root cause turns out to be broader than this one route.

- **Direct test result**: `GET /knowledge/is-strongman-safe-for-children` on the live preview returns **`HTTP 200`**, not 404. Response header `content-disposition: inline; filename="index.html"` shows Vercel is serving the plain static SPA shell for this path — the same file, byte-for-byte (identical `etag`), as every other route on the deployment, including genuinely valid article pages like `/knowledge/what-is-strongman`.
- **Root cause identified — this is a pre-existing, deployment-wide issue, not specific to this new route**: the root-level `vercel.json` (which defines the custom `routes` array pointing at `api/*.mjs` serverless functions — used for this new draft-404 rule, the pre-existing Exercise/Event true-404 mechanism, and the `start-strongman-safely` 308 redirect) does not appear to be the configuration actually governing this project's real Vercel deployments. Confirmed three ways:
  1. The **pre-existing** Exercise-library 404 mechanism (`api/library-not-found.mjs`, built and documented in an earlier priority as already working) also returns `200` for a definitely-nonexistent exercise slug, on both the preview and **production** (`educate-strong-academy.vercel.app`).
  2. Requesting `/api/library-not-found.mjs` **directly** (bypassing routing entirely) on production still returns the plain `index.html` shell, not the function's own output — meaning the function is not being invoked at all, on any path.
  3. `/knowledge/start-strongman-safely` also returns `HTTP 200`, not the intended `308` — it only "works" because the client-side `<Navigate>` fallback in `App.tsx` takes over after the page loads, which is why it visually redirects correctly for a user in a browser, but is not a genuine HTTP-level redirect.
  - The likely explanation: `frontend/vercel.json` — a separate, simpler file containing only a catch-all SPA rewrite (`{"rewrites": [{"source": "/(.*)", "destination": "/index.html"}]}`) — is the config actually in effect, which would happen if the Vercel project's dashboard "Root Directory" setting is `frontend`. That setting is only visible/changeable in the Vercel dashboard, which I don't have access to, so this is a well-evidenced hypothesis, not a confirmed fact.
- **What still holds true regardless of this bug**: the draft article remains absent from the sitemap, the Knowledge Hub listing (confirmed "24 resources", none of the retired/unpublished slugs present), prerendering, and all internal links.
- **Scope note**: fixing this properly is a Vercel project-configuration question (most likely a dashboard setting), not a code change, and affects the pre-existing Exercise/Event 404 mechanism too — this is bigger than "add one more route" and was correctly out of scope for the "do not broadly rebuild routing" instruction this round. It needs its own scoped decision once you've checked the project's Root Directory setting in the Vercel dashboard.

### Final Priority 3 status

**OPEN — one of two items verified and complete; the other verified and FAILING.** Item 1 (EatStrong disclaimer) is merged, deployed, and fully confirmed live via the public API — complete. Item 2 (Knowledge Hub true 404) is deployed but confirmed **not working**: the route returns `HTTP 200` instead of `404` on the live preview, because the project's real Vercel deployments do not appear to honour the root-level `vercel.json`'s custom routing at all (a pre-existing condition affecting the Exercise/Event 404 mechanism and the `start-strongman-safely` redirect too, not something newly broken by this round's code). Priority 3 cannot close until this is resolved — most likely via a Vercel project **Root Directory** setting change, which needs your decision and dashboard access.
...
| Knowledge Hub true-404 route (`/knowledge/is-strongman-safe-for-children`) — confirmed NOT working | **Launch blocker, needs owner action** — verified live using an owner-supplied Vercel Shareable Link: returns `HTTP 200` (same static `index.html`, same `etag`, as every other route), not `404`. Root cause: the project's real Vercel deployments do not appear to honour the root-level `vercel.json`'s custom `routes`/`api` config at all — confirmed also true of the pre-existing Exercise/Event 404 mechanism and the `start-strongman-safely` redirect (both also silently client-side-only, not real HTTP behaviour). Most likely fix is a Vercel dashboard **Root Directory** setting — see Priority 3 final verification findings above |
