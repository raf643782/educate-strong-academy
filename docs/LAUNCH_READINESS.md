...
### Stage 4A — Merge and Post-Deployment Verification (2026-07-24)

**Merge mechanism:** Stage 4A was implemented directly on `feature/libraryPages` (commit `a66efe7`), but per the standing rule that `feature/libraryPages` is never merged into `main`, the exact Stage 4A diff (9 code files, no docs) was extracted as a patch and applied cleanly onto a fresh branch cut from `main` — `production-stage4a-security-fixes` (commit `045f32d`) — mirroring the same scoped-branch pattern used for every prior production change in this programme (Viking Press correction, DB alignment, EatStrong disclaimer). Opened as [PR #4](https://github.com/raf643782/educate-strong-academy/pull/4), merged into `main` at commit `0e441db` after the owner confirmed the Render environment variables directly in the dashboard. `feature/libraryPages` was then reconciled with `main` via `git merge` (commit `dcee888`) to keep the branches in sync, matching the Priority 1 precedent.

**Deployment confirmed:** Render redeployed automatically; polled `GET /api/lessons/:id` with a real, known lesson ID (unauthenticated) until it returned `401` instead of the old `200`, confirming the new backend code is live (and, since a fail-fast `JWT_SECRET` check now runs at process startup, the fact that the service came up and served requests at all is itself confirmation `JWT_SECRET` is correctly set in production). Vercel redeployed the frontend from `main` automatically on the same merge.

**Read-only production verification results, using only existing data/accounts, no writes made:**

1. **Backend health returns 200** — confirmed (`GET /api/health`).
2. **Public published Courses still load** — confirmed. Fetched `/api/courses` (5 published courses returned) and `/api/courses/level-1-coaching-strongman` in full (7 modules, all lesson metadata) — unchanged from pre-fix behaviour.
3. **Unpublished Courses no longer publicly exposed** — **unverified**. No unpublished course currently exists in the production catalogue (all 5 known courses are published) and finding one requires ADMIN API access this environment doesn't have. Confirmed only that a nonexistent slug still returns 404 (unchanged, doesn't isolate the new `isPublished` branch specifically). Recommend the owner spot-check this directly if an unpublished/draft course is ever created.
4. **Unauthenticated lesson requests return 401** — confirmed, using a real lesson ID from the published Level 1 Coaching course.
5. **Non-enrolled learner receives 403** — **unverified**. Requires a real LEARNER login this environment does not have and was not authorised to create.
6. **Enrolled learner can still load their lesson** — **unverified** for the same reason. (Confirmed via the public API that exactly one real enrolment exists in production, on the Level 1 Coaching course — the owner or that learner could complete this specific check directly.)
7. **ADMIN can still load lesson content** — **unverified**, same reason — requires real ADMIN credentials.
8. **Public Portal Preview link is gone** — confirmed live on `https://educate-strong-academy.vercel.app/login`.
9. **Unauthenticated preview routes go to Login** — confirmed live: `/portal-preview/admin` with no token redirects to `/login`.
10. **Non-ADMIN users return to their own dashboard** — **unverified** on production (confirmed pre-merge via a local mock server instead — see the Stage 4A section above); needs a real non-ADMIN login to confirm on the live site.
11. **ADMIN preview access still works** — **unverified** on production for the same reason; confirmed pre-merge via the local mock.
12. **Login, registration and password reset remain functional** — confirmed for login (`POST /auth/login` with deliberately wrong credentials against a nonexistent test email → clean `401 Invalid credentials`, proving the endpoint runs normally end-to-end post-JWT-refactor) and forgot-password (`POST /auth/forgot-password` with a nonexistent test email → the expected neutral anti-enumeration message, no data written). Registration was verified structurally only — the `/register` page loads and renders its form correctly — and was **not submitted**, since a real registration is itself a production write and this stage's instruction was explicit not to create test scenarios that write to production.
13. **Invalid token handling clears authentication and returns to Login** — confirmed live, end-to-end, on the real production frontend and backend: set a deliberately garbage token in `localStorage`, navigated to `/dashboard`, confirmed the app redirected to `/login` with the token cleared (`localStorage.getItem('es_token') === null`).

**Net result: 8 of 13 checks fully confirmed live in production.** The remaining 5 (items 3, 5, 6, 7, 10, 11 — unpublished-course exposure, non-enrolled learner, enrolled learner, ADMIN lesson access, non-ADMIN dashboard redirect, ADMIN preview access) require real account credentials or an unpublished course this environment does not have and was correctly not authorised to fabricate.

**Status: accepted by the owner as deferred user acceptance tests (2026-07-24).** These 5 items do not block further Priority 4 work — recorded here so they aren't lost, to be confirmed by the owner directly with their own accounts/data at a convenient time, not because there is any indication of a problem.
...
| Public "Preview the portals" link on the Login page | ~~Client decision~~ **Resolved and merged to `main` 2026-07-24 (Priority 4, Stage 4A)** — link removed from Login; all `/portal-preview/*` routes now require ADMIN authentication; tooling itself retained, not deleted. **Verified live in production**: link absent, unauthenticated `/portal-preview/*` redirects to `/login` |
| `GET /api/lessons/:id` had no auth or enrolment check (full lesson content, video/resource URLs exposed to anyone) | ~~Critical security risk~~ **Resolved and merged to `main` 2026-07-24 (Priority 4, Stage 4A)** — now requires authentication + enrolment (or ADMIN), matching the existing `documents.ts` pattern. **Verified live in production**: an unauthenticated request against a real lesson ID now returns 401 |
| `GET /api/courses/:slug` had no `isPublished` filter (unpublished/draft course metadata publicly fetchable) | ~~Security risk~~ **Resolved and merged to `main` 2026-07-24 (Priority 4, Stage 4A)** — unpublished courses now return 404 on the public route; ADMIN editing routes unaffected. Live verification of the unpublished branch itself is outstanding — no unpublished course currently exists in production to test against; see the Stage 4A post-deployment section |
| `JWT_SECRET` fell back to a fixed, guessable literal (`'fallback-secret'`) if unset | ~~Security risk~~ **Resolved and merged to `main` 2026-07-24 (Priority 4, Stage 4A)** — centralised in `backend/src/config/jwtSecret.ts`, fails fast at startup in production if unset, never falls back silently. **Verified live**: the service serving requests post-deploy is itself proof `JWT_SECRET` is correctly set (owner also confirmed directly in the Render dashboard) |
| No frontend handling for an expired/invalid auth token | ~~Recommended improvement~~ **Resolved and merged to `main` 2026-07-24 (Priority 4, Stage 4A)** — global 401 interceptor added, excludes public auth pages to avoid redirect loops. **Verified live in production**: an invalid token is cleared and the user redirected to `/login`, with no loop on the login page itself |
