...
**Net result: 8 of 13 checks fully confirmed live in production; 5 remain unverified** (items 3, 5, 6, 7, 10, 11) purely because they require either an unpublished course or real non-owner-known account credentials (LEARNER-non-enrolled, LEARNER-enrolled, ADMIN, non-ADMIN-authenticated) that this environment does not have and was correctly not authorised to fabricate. All 5 are things the owner can confirm directly with their own accounts in a couple of minutes; none of them point to any evidence of a problem — they simply couldn't be exercised without credentials this session doesn't hold.
