...
|---|---|---|---|---|
| `VITE_SITE_URL` | **YES** | No | The canonical production domain — **owner decision required**: `https://educatestrong.com` or `https://www.educatestrong.com` (both are in the CORS allowlist; owner must choose one canonical form). Controls canonical tags, sitemap URLs, and Open Graph images. | Canonical tags and sitemap will reference the old Vercel preview URL. SEO impact. |
...
| `VITE_SANITY_USE_CDN` | When Sanity is activated | No | Set to `false` to bypass the Sanity CDN and hit the live API directly (useful during content previews/testing). Defaults to `true` (CDN on) if unset. | Defaults to CDN on. Changing this after Sanity is active requires a new Vercel deployment. |
...
| `FRONTEND_URL` | **YES** | No | Full URL of the canonical production frontend — **owner decision required** (see canonical domain note below). Used in password-reset and email-verification links. **Must be updated when the domain changes.** | Password reset and verification emails contain links pointing to the wrong domain. |
...
| `EMAIL_FROM` | Before emails go live | No | Sender address on the verified Resend subdomain, e.g. `EducateStrong Academy <no-reply@send.educatestrong.com>`. Must match the domain verified in Resend (see `platform-resend.md`). | Falls back to `onboarding@resend.dev` (Resend test address). Not suitable for production. |
