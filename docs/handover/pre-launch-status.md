...
**Owner decision required**: decide whether the canonical URL will be `https://educatestrong.com` (no www) or `https://www.educatestrong.com`. Both forms are already in the backend CORS allowlist — no code change is needed for either. The chosen canonical form must be set consistently in `VITE_SITE_URL` (Vercel) and `FRONTEND_URL` (Render).

| Check | Status |
|---|---|
| `VITE_SITE_URL` set in Vercel | **Not set** — owner must decide canonical form, then set it before domain migration |
...
| CORS allowlist | `educatestrong.com` and `www.educatestrong.com` are already in the backend CORS allowlist — no code change needed for either choice |
| `FRONTEND_URL` on Render | Must be set to the canonical URL (matching `VITE_SITE_URL`) — currently controls email link domain |

**Action required**: Owner decides canonical URL form, then sets `VITE_SITE_URL` in Vercel and `FRONTEND_URL` in Render consistently. Trigger a fresh Vercel deployment after setting.
...
| Knowledge Hub public source | 21 hardcoded articles from `frontend/src/data/knowledgeArticles.ts` — fully functional |
| Sanity Studio | Exists in `sanity/` directory of `feature-sanity-stage5a-safety` branch; project ID `ut2wo29d` referenced — not yet ported to main |
| Sanity preview route | `/knowledge-hub-preview` — ADMIN-protected, noindex, shows "not configured" message until env vars set |
| 9 Sanity articles | Exist in the approved manifest (`approvedKnowledgeArticles.ts`) — no Sanity project connected |
| Launch risk | None — the existing Knowledge Hub works independently of Sanity |

**Post-launch action**: Port Sanity Studio from feature branch, create/access Sanity project `ut2wo29d`, set env vars in Vercel, load articles, review at `/knowledge-hub-preview` (ADMIN login required), resolve all 21 URL migration decisions (see `knowledge-sanity-migration-map.md`), then do the cutover. See `docs/handover/cms-guide.md`.
...
| `EMAIL_FROM` on Render | **Not set** — must be set to `EducateStrong Academy <no-reply@send.educatestrong.com>` once Resend subdomain is verified |
| `NOTIFICATIONS_EMAIL` on Render | **Not set** |
| Sending domain verified in Resend | **Not done** — subdomain `send.educatestrong.com` must be verified |
