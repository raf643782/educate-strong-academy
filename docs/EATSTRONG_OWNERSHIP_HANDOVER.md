...
**Priority 3 status: OPEN, not complete.** The item tracked in this document (Section 5, `nutrition-conversations-with-athletes` disclaimer) is now **resolved and verified** — [PR #3](https://github.com/raf643782/educate-strong-academy/pull/3) was approved, merged into `main` (commit `9fc9e36`), and the live production API confirms the disclaimer is now present on exactly that one article, with all 11 articles and every other field unchanged. Priority 3 as a whole remains open only because of a separate, unrelated Knowledge Hub item (a true-404 route for an unpublished draft article), which has now been verified live and confirmed **not working** — see `docs/LAUNCH_READINESS.md` for the full finding, including a likely Vercel project-configuration root cause. Nothing further is required from an EatStrong perspective for Priority 3 closure.
...
- ~~**`nutrition-conversations-with-athletes`** is missing its `scopeOfPracticeNote` disclaimer in the live production database~~ **RESOLVED AND VERIFIED 2026-07-23.** A guarded, data-only migration (`backend/prisma/migrations/20260723180000_correct_nutrition_conversations_scope_note/migration.sql`), created on branch `production-eatstrong-scope-note-correction` (from `main`) and opened as [PR #3](https://github.com/raf643782/educate-strong-academy/pull/3), was approved and merged into `main` (commit `9fc9e36`). Render auto-deployed and ran the migration via its existing `prisma migrate deploy` build step — no credential requested or handled at any point. The live `GET /api/be-strong/articles/nutrition-conversations-with-athletes` endpoint now returns the standard disclaimer text verbatim; all 11 EatStrong articles remain present; 3 other articles spot-checked confirm no other row was touched; `GET /api/health` returns `200`. This article's `authorName` ("EatStrong Editorial Team") and `reviewerName` ("Victoria Wilson") are unchanged by this migration and remain exactly as flagged below.
