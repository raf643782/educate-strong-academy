...
**Priority 3 status: OPEN, not complete.** The one remaining open item tracked in this document (Section 5, `nutrition-conversations-with-athletes` disclaimer) has a migration drafted and a pull request open ([PR #3](https://github.com/raf643782/educate-strong-academy/pull/3)), but it has not been merged or verified live. Priority 3 will be marked complete only once that PR is approved, merged, and re-verified against the live API.
...
- **`nutrition-conversations-with-athletes`** is missing its `scopeOfPracticeNote` disclaimer in the **live production database** — every other FREE article has one, this one still has `null` there. **Update 2026-07-23 (closure pass):** a guarded, data-only migration (`backend/prisma/migrations/20260723180000_correct_nutrition_conversations_scope_note/migration.sql`) has been created on a scoped branch (`production-eatstrong-scope-note-correction`, branched from `main`) and opened as [PR #3](https://github.com/raf643782/educate-strong-academy/pull/3) into `main` — **left unmerged, pending your explicit approval.** The migration updates `BeStrongArticle.scopeOfPracticeNote` only where `slug = 'nutrition-conversations-with-athletes'` AND the current value `IS NULL`; it aborts and rolls back if zero or more than one row matches. No schema change, no other row touched, no credential requested or handled. **This item remains open** until the PR is approved, merged, and the live API is re-checked to confirm exactly one row changed.
