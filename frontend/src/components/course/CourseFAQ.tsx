            ...
            {faqs.map((faq, idx) => (
              <div key={idx} className="rounded-lg overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
                ...
                >
                  ...
                  <svg aria-hidden="true" className={`w-5 h-5 text-es-muted flex-shrink-0 mt-0.5 transition-transform duration-200 ${open === idx ? 'rotate-180' : ''}`}
                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {open === idx && (
                  <div id={`faq-panel-${idx}`} className="px-5 pb-4 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.07)', background: '#1B1B20' }}>
