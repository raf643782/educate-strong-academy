 ...
 * article route, or sitemap generator must check a slug against this
 * explicit allow-list — never query Sanity by status alone.
 *
 * This manifest reflects the final approved Knowledge Hub editorial review
 * only. It does not fetch anything from Sanity itself (see frontend/src/lib/sanity.ts
 * for the actual data-fetching layer) — it is the static decision of *which*
 * slugs that layer is allowed to serve publicly.
 *
 * Scope notes:
 * - Only the 9 approved slugs below may ever be listed, routed to, or
 *   included in a sitemap for the Sanity-backed Knowledge Hub.
 * - The 14 other pre-existing hardcoded Knowledge Hub articles (in
 *   frontend/src/data/knowledgeArticles.ts, served at /knowledge/:slug) are
 *   a separate system entirely and are not migrated, referenced, or
 *   affected by this manifest in any way.
...
export type DateStatus = 'unverified' | 'confirmed';
...
export interface ApprovedKnowledgeArticleMeta {
  ...
  slug: string;
  /** Always true for entries in APPROVED_KNOWLEDGE_ARTICLES; kept explicit for safety at call sites. */
  isPubliclyApproved: boolean;
  /** Always false for entries in APPROVED_KNOWLEDGE_ARTICLES; true only for EXCLUDED_DRAFT_ARTICLES. */
  isDraftOnly: boolean;
  seoTitle: string;
  ...
  publishedDateStatus: DateStatus;
  lastReviewedDateStatus: DateStatus;
}
...
/**
 * Draft-only articles — must NEVER appear in a public listing, public
 * article route, navigation, internal link, or sitemap. Kept here (rather
 * than simply omitted) so the exclusion is explicit and traceable, not a
 * silent absence someone could "fix" by accident.
 */
export const EXCLUDED_DRAFT_ARTICLES: ApprovedKnowledgeArticleMeta[] = [
  {
    title: 'Is Strongman Safe for Children? A Careful Guide for Parents, Coaches, and Clubs',
    slug: 'is-strongman-safe-for-children',
    isPubliclyApproved: false,
    isDraftOnly: true,
...
const APPROVED_SLUG_SET: ReadonlySet<string> = new Set(
  APPROVED_KNOWLEDGE_ARTICLES.map((article) => article.slug)
);
...
/** The exact 9 approved public slugs — safe to use directly for listing pages and sitemap generation. */
export function getApprovedKnowledgeSlugs(): string[] {
  return APPROVED_KNOWLEDGE_ARTICLES.map((article) => article.slug);
}

/** Looks up manifest metadata for an approved slug only. Returns undefined for anything not explicitly approved, including the excluded draft. */
export function getApprovedKnowledgeArticleMeta(
  slug: string
): ApprovedKnowledgeArticleMeta | undefined {
  return APPROVED_KNOWLEDGE_ARTICLES.find((article) => article.slug === slug);
}

/** True only for the excluded draft-only slug(s) — useful for defensive checks in routing/link code. */
export function isExcludedDraftKnowledgeSlug(slug: string): boolean {
  return EXCLUDED_DRAFT_ARTICLES.some((article) => article.slug === slug);
}
