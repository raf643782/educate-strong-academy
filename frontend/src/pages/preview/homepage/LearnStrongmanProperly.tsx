/**
 * LearnStrongmanProperly — preview-only combined Knowledge Hub, Exercise
 * Library and Event Library section.
 *
 * This section did not exist on production Home in this form (Exercise
 * Library and Event Library currently have no homepage presence at
 * all, despite holding the richest, most specific content on the
 * site). The Knowledge Hub cards reuse the same three real articles
 * already shown in production's KnowledgeHubPreview (data/knowledgeArticles.ts,
 * a static data file, unchanged). The Exercise and Event cards fetch
 * real, live entries from the same API the dedicated library pages
 * use, so this is proof of depth rather than another written claim.
 */
      ...
      <div className="grid gap-3 mb-5">
        ...
        {state === 'error' && (
          <p className="text-xs text-white/30 italic">
            ...
            not load in this preview session.
          </p>
        )}
      </div>
...
export default function LearnStrongmanProperly() {
  return (
    <section
      style={{
        background: [
            ...
            {KNOWLEDGE_PREVIEW.map((article) => (
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <h3 className="font-bold text-[#F5F5F7] text-base leading-snug mb-3 flex-1">{article.title}</h3>
                  <p className="text-sm text-[#75757D] leading-relaxed mb-5">{article.summary}</p>
                  <div className="flex items-center justify-between mt-auto">
                    <span className="text-xs text-[#5A5A62]">{article.readTime} read</span>
                    <span className="text-xs font-semibold" style={{ color: '#C2186A' }}>Read Article →</span>
                  </div>
                </div>
              </Link>
            ))}
